 ...
 * 
 * Quy trình:
 * 1. Đọc file Drawing.txt từ thư mục DATA
 * 2. Kiểm tra số (phải đúng 6 chữ số)
...
(function () {
    ...
    try {
        ...
        if (!dataFolder.exists) {
            throw new Error("Không tìm thấy thư mục DATA tại:\n" + dataFolder.fsName);
        }

        var drawingFile = new File(dataFolder.fsName + "/Drawing.txt");
        if (!drawingFile.exists) {
            throw new Error("Không tìm thấy file Drawing.txt tại:\n" + drawingFile.fsName);
        }

        drawingFile.open("r");
        var content = drawingFile.read();
        drawingFile.close();
