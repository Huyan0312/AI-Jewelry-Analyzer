...
// 💾 SAVE CHECK TO FILE
// ====================================
function saveCheckToFile(status) {
    try {
        // Tìm folder DATA để lưu file check.txt (cùng cấp với script)
        ...
        // Tạo file check.txt trong folder DATA
        var checkFile = new File(dataFolder.fsName + "/check.txt");
        var content = status;
        checkFile.open("w");
        checkFile.write(content);
        checkFile.close();
    } catch (e) {
        // Nếu không lưu được, bỏ qua
    }
}
...
(function () {
    // Khởi tạo file check.txt là false (chưa nhập liệu)
    saveCheckToFile("false");
    ...
    try {
        ...
        function showDirectionDialog() {
            ...
            function closeDialog(dir) {
                ...
                // Đã nhập liệu hợp lệ -> Lưu true vào check.txt
                saveCheckToFile("true");
