 ...
 * Photoshop CS5+ — Copy View 1, add chain và position cùng với Shape 1, Shape 2
 * 
 * Workflow:
 * - Copy View 1 tại chỗ
 * - Tìm Shape 1 và Shape 2 (đi chung với View 1) — trước đây Shape 8/9
 * - Link View 1 copy với Shape 1 và Shape 2
 * - Position: sát trái Bounding Box Stroke (cách 50px) - di chuyển cả 3 layers cùng nhau
 * - Add chain từ DATA/{brand}/ và canh trục với Shape 1 và Shape 2
 */
...
(function () {
    ...
    try {
        ...
        // Tìm layer View 1 (FRONT — sau Scale/AI)
        ...
        // Tìm Shape 1 và Shape 2 (trước đây Shape 8/9)
        var shape8 = findLayerByName(doc, "shape 1");
        var shape9 = findLayerByName(doc, "shape 2");

        if (!shape8) {
            throw new Error("Không tìm thấy layer 'Shape 1'.");
        }
        if (!shape9) {
            throw new Error("Không tìm thấy layer 'Shape 2'.");
        }

        // Copy View 1 tại chỗ
        var layer1Copy = duplicateLayer(layer1, "View 1 copy");
        if (!layer1Copy) {
            throw new Error("Không thể copy View 1.");
        }

        // Link View 1 copy với Shape 1 và Shape 2
        linkLayers([layer1Copy.name, shape8.name, shape9.name]);

        // Position View 1 copy: sát trái Bounding Box Stroke (cách 50px)
        // Di chuyển View 1 copy sẽ di chuyển cả 3 layers cùng nhau (vì đã link)
        positionLayer1CopyLeftOfBoundingBox(layer1Copy, boundingBoxLayer, 50);

        // Add chain và canh trục với Shape 1 và Shape 2
        addChainAndPosition(shape8, shape9);

        // Xóa Shape 1 và Shape 2 sau khi đã position chain xong
    ...
    function positionLayer1CopyLeftOfBoundingBox(sourceLayer, targetLayer, gap) {
        ...
        // Lấy bounds của source layer (View 1 copy)
    ...
    function readBrandFromFile() {
        try {
            ...
            if (!cacheFile.exists) {
                throw new Error("Không tìm thấy file cache tại: " + cacheFile.fsName);
            }
            cacheFile.open("r");
            ...
            for (var i = 0; i < lines.length; i++) {
                ...
                if (line && line.indexOf("brand=") === 0) {
                    ...
                    if (brandName.toUpperCase() === "NONE") return "NONE";
            ...
            throw new Error("Lỗi khi đọc cache: " + e.message);
        }
    }
    ...
    function addChainAndPosition(shape8, shape9) {
        try {
            ...
            // Định vị chain với Shape 1 (tương tự 4.1.Autochain.jsx)
            ...
            // Định vị chain copy với Shape 2
